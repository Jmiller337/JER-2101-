...
- **Motion.** Only to explain a change: the switch knob slides, the shutter shrinks when pressed. Everything stops with Reduce Motion.
...
2. **Hierarchy.** App mark and name (Title 1); one line of what it does (Body, secondary); the Start button filling the rest of the screen ("Start. Tap anywhere.", Large Title weight).
...
3. **Layout.** Two tinted cards splitting the space under the title, matching the spoken instruction "top half for yes, bottom half for no".
...
3. **Layout.** A single grouped card on the grouped background.
