...
- **Page box.** The camera draws a box around the page it sees: its four corners, so it turns with a tilted page. White while the page is being lined up, green when the picture is taken. The detector fills the holes the text leaves in the page's bright region, which lets it find tilted pages at any angle. The picture now sits above the action panel and is shown whole, so the box and the photo match. Unit tests check the corners at angles from -15 to 60 degrees (within 2.5% of the picture); e2e tests check that the box lines up with the page on screen and follows a tilted page.
- **Non-ideal framings.** The forgiving capture now waits 1.5 seconds instead of 3, and also takes dim pages (brightness 55 to 70, which are brightened before sending) and small ones. New tests: a tilted page running off the picture, a page that goes soft, and a dim page are each taken once the phone is calm; a portrait fake-camera video of a tilted, cut-off page is captured end to end.
- **Modes, like the iPhone's Camera.** By the owner's request the camera screen has a mode strip, PDF, Camera, and Photos, above one action panel. Swipe left or right anywhere (or tap a mode) to switch, and the app says the mode's name and what to do. Photos reads a photo or screenshot from the library. "Open a PDF" moved out of More into the PDF mode. The modes are tabs for VoiceOver users, who cannot swipe here. Framing guidance and automatic capture run only in Camera mode. The swipe is mentioned in "Camera ready." until the user has changed mode once.
- **Not verified on a real iPhone:** that the box sits on the page with a real camera, that the forgiving capture is not too eager (taking a picture while the user is still moving toward the page), and that the swipe feels right with Safari's own back-swipe from the screen edge.

## Liquid Glass

- **The owner asked for Liquid Glass** (iOS 26's material) as the design theme. Bars, cards, and buttons are now translucent glass with a sheen, a rim of light, and a soft shadow; the primary action on each screen is tinted glass; items inside a glass bar have no fill of their own; buttons are capsules; presses squash and spring back; the camera's modes sit in a glass capsule with a sliding lens; the first-launch screens have a soft coloured background for the glass to bend. Settings and Ask have a floating glass bar with Done and a checkmark, and content fades out above it.
...
## No random pictures, full-screen camera

- **Automatic capture only takes pages with writing.** The owner saw random pictures. Two causes were likely: anything bright or detailed counted as a page (a wall, a window, a patterned table, a blank sheet), and the page just read was taken again when the camera reopened. Now both automatic paths need writing on the page, measured inside its outline: some ink, not mostly ink, some smooth paper, and paper light enough. Detail running off three sides with no paper round it is a patterned surface, not a page. After a picture the view must change before another automatic one ("This is the page you just read."). The forgiving wait went back up from 1.5 to 2 seconds. New unit tests stare at a blank sheet, a keyboard, a busy pattern, and a rough texture for six seconds without a picture; new e2e tests show a blank sheet is never photographed (but Capture still works) and the page just read is not taken again after New document.
