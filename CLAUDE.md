...
- **Design.** `docs/DESIGN.md` is the per-screen spec (Apple-style: content first, restraint, glass only for floating controls). Each screen shows only its core controls; everything else sits behind a "More" button (`MoreButton` in `src/components/ui.tsx`, a disclosure with `aria-expanded`). Reading: Back, Play or Pause, Forward, New document, Ask a question, More. Camera: the picture with the page box, a status capsule, More (Use phone camera instead, Settings), the mode strip (PDF, Camera, Photos), and one action panel. The camera never tells the user where to put the phone: it says "Camera ready." and then only cues. Settings and Ask have Done in a navigation bar. `tests/e2e/simple.spec.ts` pins these sets; add new controls behind More unless they are used on almost every document.
