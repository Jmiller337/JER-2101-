...
- **Design tokens** are the `@theme` block in `src/app/globals.css` (navy, slate, amber; every text and background pair at 7:1 or better); icons are inline SVGs in `src/components/icons.tsx`, always decorative and next to a visible label.
