...
- **Minimum iOS is 16.4**, not 16.0. Next.js 16 compiles for Safari 16.4 and newer by default and Tailwind CSS v4 targets the same version; the `browserslist` in `package.json` states it explicitly. Every iPhone that runs iOS 16 can update to 16.4 or later. Newer features (`ImageCapture` from iOS 18.4, `navigator.audioSession` from iOS 17, wake lock) are feature-detected.
